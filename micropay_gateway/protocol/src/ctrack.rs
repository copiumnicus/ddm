...
pub struct ClientRisk<U, K> {
    ...
    pub vendor_client_expand_risk: u64,
}
...
impl<U, K> ClientRisk<U, K> {
    pub fn new(o: Box<dyn ChainOracle<U, K>>, expand_risk: Option<u64>) -> Self {
        Self {
            o,
            vendor_client_expand_risk: expand_risk.unwrap_or(DEFAULT_VENDOR_CLIENT_EXPAND_RISK),
        }
    }
    pub fn get_client_risk_adj_collateral(&self, ci: &U) -> Result<u64, OracleErr> {
        ...
        let expand_risk = self.vendor_client_expand_risk;
        let unspent_per_vendor_safe = collat / (total_subs + expand_risk);
        Ok(unspent_per_vendor_safe)
    }
}
...
impl<V, U, K> CreditTrack<V, U, K> {
    ...
    pub fn user_credit(&self, ci: &U) -> Result<UserCredit, OracleErr> {
        ...
        let locked = self.u.locked_cost(ci);
...
pub struct UserCredit {
    ...
    pub unmarked: u64,
    /// in case of parallel this becomes relevant to reduce user avb credit
    pub locked: u64,
